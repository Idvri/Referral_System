...
class UserVerificationAPIView(APIView):
    ...
    def post(self, request):
        ...
        number = serializer.validated_data.get('number')
        user = {'number': number, 'verify_code': serializer.validated_data.get('verify_code')}

        if user != request.session.get('user') or not request.session.get('user'):
            return Response(
                {'number_or_verify_code': 'Указан неправильный номер телефона или код для входа!'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class UserRetrieveAPIView(generics.RetrieveAPIView):
    ...
    def post(self, request, pk):
        ...
        if not invite_code:
            return Response(
                {'referral_code': 'Указан недействительный инвайт-код!'},
                status=status.HTTP_400_BAD_REQUEST
            )
